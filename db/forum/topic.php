...
WHERE
    ...
    $topicID = $_GET['id'];
        ...
        if(!$result)
        {
            echo 'The topics could not be displayed, please try again later.' . mysqli_error();
        }
		...
		else
		{
		    ...
		    else
		    {
		        ...
		        if(!$result)
		        {
		            echo 'The topics could not be displayed, please try again later.' . mysqli_error($dbconfig);
		        }
		        else
		        {
		            if(mysqli_num_rows($result) == 0)
    ...
    echo "
    <h3>Currently logged in as '" . $_SESSION['globalID'] .
